"""Queue worker for processing Twitter fetch jobs."""
...
from src.scraper.mappers import TwitterPostMapper
...
class TwitterQueueWorker:
    """Worker for processing Twitter fetch jobs from BullMQ."""
    ...
    def start(self) -> None:
        ...
        logger.info(
            f"Starting Twitter queue worker for queue: {self.settings.twitter_fetcher_queue}",
        )

        self.worker = Worker(
            self.settings.twitter_fetcher_queue,
            self._process_job,
            {
        ...
        logger.info("Twitter queue worker started successfully")

    async def _process_job(self, job: Job, token: str) -> None:
        """
        Process a single Twitter fetch job.
        ...
        logger.info(f"Processing Twitter fetch job {job_id}")
        ...
        try:
            ...
            if job_data.sourceType != "twitter":
                raise ValueError(
                    f"Invalid source type: {job_data.sourceType}. Expected 'twitter'",
                )

            logger.info(
                f"Fetching Twitter posts for @{job_data.externalId} "
                f"(sourceId={job_data.sourceId}, limit={job_data.limit}, cursor={job_data.cursor})",
            )

            # Fetch posts using scraper
            posts, next_cursor = await self.scraper.fetch_user_tweets(
            ...
            for post in posts:
                fetched_post, media_jobs = TwitterPostMapper.to_fetched_post(
                    post,
    ...
    async def stop(self) -> None:
        ...
        logger.info("Stopping Twitter queue worker...")
        ...
        logger.info("Twitter queue worker stopped")
