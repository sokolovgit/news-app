...
class ResultPublisher:
    ...
    async def publish_success(
        ...
        Args:
            source_id: Source ID
            source_type: Source type ("twitter")
    ...
    def _get_error_code(error: Exception) -> str:
        ...
        # Check for rate limit patterns first
        rate_limit_patterns = [
            "rate limit",
            "ratelimiterror",
            "429",
        ...
        # Check for specific Twitter exceptions
        if "AccountNotFoundError" in error_name or "not found" in error_message:
            return "PROFILE_NOT_FOUND_ERROR"
        if "AuthError" in error_name or "authentication" in error_message or "401" in error_message:
            return "AUTH_ERROR"
    ...
    def _is_retryable_error(error: Exception) -> bool:
        ...
        if any(
            pattern in error_message or pattern in error_name.lower()
