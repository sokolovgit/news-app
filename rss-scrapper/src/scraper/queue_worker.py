"""Queue worker for processing RSS fetch jobs."""
...
class RssQueueWorker:
    """Worker for processing RSS fetch jobs from BullMQ."""
    ...
    def start(self) -> None:
        ...
        logger.info(
            f"Starting RSS queue worker for queue: {self.settings.rss_fetcher_queue}",
        )

        self.worker = Worker(
            self.settings.rss_fetcher_queue,
            self._process_job,
            {
        ...
        logger.info("RSS queue worker started successfully")

    async def _process_job(self, job: Job, token: str) -> None:
        """
        Process a single RSS fetch job.
        ...
        logger.info(f"Processing RSS fetch job {job_id}")
        ...
        try:
            ...
            if job_data.sourceType != "rss":
                raise ValueError(
                    f"Invalid source type: {job_data.sourceType}. Expected 'rss'",
                )

            logger.info(
                f"Fetching RSS feed {job_data.externalId} "
                f"(sourceId={job_data.sourceId}, limit={job_data.limit}, cursor={job_data.cursor})",
            )

            # Fetch RSS feed using scraper
            entries, feed_info, next_cursor = self.scraper.fetch_feed(
                feed_url=job_data.externalId,
                limit=job_data.limit,
                cursor=job_data.cursor,
            )

            # Map entries to FetchedPost format and collect media upload jobs
            fetched_posts = []
            all_media_jobs = []

            for entry in entries:
                fetched_post, media_jobs = RssEntryMapper.to_fetched_post(
                    entry,
                    feed_info,
    ...
    async def stop(self) -> None:
        ...
        logger.info("Stopping RSS queue worker...")
        ...
        logger.info("RSS queue worker stopped")
