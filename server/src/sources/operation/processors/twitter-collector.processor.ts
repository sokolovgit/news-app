...
@Processor(SourceQueue.TWITTER_FETCHER)
export class TwitterCollectorProcessor extends WorkerHost {
  constructor(
  ...
  async process(job: Job<CollectorJobData>): Promise<void> {
    ...
    this.logger.debug(
      `Processing Twitter collector job ${id} for source ${data.sourceId}`,
    );
    ...
    try {
      ...
      this.logger.debug(
        `Successfully processed Twitter collector job ${id} for source ${data.sourceId}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to process Twitter collector job ${id} for source ${data.sourceId}: ${error instanceof Error ? error.message : String(error)}`,
